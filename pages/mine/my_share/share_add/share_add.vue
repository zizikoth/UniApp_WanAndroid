<template>
	<view class="page">
		<view class="share-container">

			<text class="share-label">标题</text>
			<u-input class="share-input" v-model="title" type="textarea" :border="true" border-color="#333333"
				:clearable="true" placeholder="请输入标题" :trim="true" />
			<text class="share-label">链接</text>
			<u-input class="share-input" v-model="link" type="textarea" :border="true" border-color="#333333"
				:clearable="true" placeholder="请输入链接" :trim="true" />

			<view class="share-submit" @click="submit">提交</view>

		</view>
	</view>
</template>

<script>
	import api from '@/http/ApiService.js'
	import checker from '@/utils/Checker.js'
	var self
	export default {
		onLoad() {
			self = this
		},
		data() {
			return {
				title: '',
				link: ''
			}
		},
		methods: {
			submit() {
				console.log(self.title, self.link)
				if (checker.checkShareAdd(self.title, self.link)) {
					api.myShareAdd(self.title, self.link).then(res => {
						getApp().globalData.refresh.shareAdd = true
						uni.navigateBack({})
					})
				}
			}
		}
	}
</script>

<style>
	.share-container {
		display: flex;
		flex-direction: column;
		width: calc(100% - 60rpx);
		margin-left: 30rpx;
		margin-right: 30rpx;
	}

	.share-label {
		display: flex;
		padding-left: 20rpx;
		margin-bottom: 30rpx;
		height: 40rpx;
		color: #333333;
		font-size: 30rpx;
		border-left: solid #333333 10rpx;
	}

	.share-input {
		display: flex;
		margin-bottom: 30rpx;
	}

	.share-submit {
		display: flex;
		width: 100%;
		height: 80rpx;
		border-radius: 10rpx;
		background-color: #000000;
		margin-bottom: 30rpx;
		margin-top: 30rpx;
		color: #ffffff;
		font-size: 34rpx;
		align-items: center;
		justify-content: center;
	}
</style>
